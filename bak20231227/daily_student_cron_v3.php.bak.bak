function daily_student_cron_v3_func() {
...
// $validReplacementPeriod = 'Y';
	...
	if ($userName != '') {
		$content 			.= "<h3>Daily Student Cron Process Executed by $userName</h3>";
	} else {
		$content			.= "<h3>Daily Student Cron Process Automatically Executed</h3>";
		$userName			= "CRON";
		$dst				= date('I');
		if ($dst == 0) {
...
//							$content				.= "Student $student_call_sign is on hold<br />";
					...
					if ($doDebug) {
						...
						echo "</pre><br />";
						$myInt		= count($replaceArrayAdvanced);
							...
							if ($numACRows > 0) {
								foreach ($wpw1_cwa_advisorclass as $advisorClassRow) {
									...
									if ($advisorEmail === NULL) {
										...
										$myError		= $wpdb->last_error;
										if ($doDebug) {
											echo "attempting to get the email address for $replacement_call_sign failed<br />
													SQL: $myQuery<br />
													Error: $myError<br />";
										}
										...
										if ($doDebug) {
											echo "got email of $advisorEmail for $replacement_call_sign<br />";
										}
									...
									if ($callSign1 != '') {
										...
										if ($doDebug) {
											echo "$replaceCallSign (1) works as a replacement<br />";
										}
									} elseif ($callSign2 != '') {
										...
										$gotAReplacement		= TRUE;
										if ($doDebug) {
											echo "$replaceCallSign (2) works as a replacement<br />";
										}
									} elseif ($callSign3 != '') {
										...
										$gotAReplacement		= TRUE;
										if ($doDebug) {
											echo "$replaceCallSign (3) works as a replacement<br />";
										}
									}
		
									if ($gotAReplacement) {
										if ($doDebug) {
											echo "Have a replacement $replaceCallSign. Getting student record<br />";
										}
										...
										if ($wpw1_cwa_replace === FALSE) {
											...
											if ($doDebug) {
												$myStr			= $wpdb->last_query;
												echo "ran $myStr<br />and retrieved $numRRows rows from $studentTableName table<br />";
											}
											if ($numRRows > 0) {
												if ($doDebug) {
													$myStr			= $wpdb->last_query;
													echo "ran $myStr<br />and found $numRRows rows in $studentTableName<br />";
												}
												...
												if ($addResult[0] === FALSE) {
													...
													if ($doDebug) {
														echo "did unset replaceArray$replacement_level $replaceSequence|<br />";
													}
												}
...
//																}
													...
													if ($student_no_catalog == 'Y') {
														if ($doDebug) {
															echo "student_no_catalog is 'Y'<br />";
														}
														if ($sendEmail) {
															if ($doDebug) {
...
//												$content		.= "Reminder successfully added<br />";
										...
										if ($wpw1_cwa_information === FALSE) {
											...
											if ($doDebug) {
												$myStr			= $wpdb->last_query;
												echo "ran $myStr<br />and found $numIRows rows in the combined tables<br />";
											}
											if ($numIRows > 0) {
												foreach ($wpw1_cwa_information as $infoRow) {
													...
													if ($advisorClass_level != $student_level) {
														if ($doDebug) {
															echo "student level of $student_level supposedly in $str_assigned_advisor $str_assigned_advisor_class class doesn't match the class level of $advisorClass_level<br />";
														}
														...
														if ($wpw1_cwa_replace === FALSE) {
															...
															if ($numRRows > 0) {
																...
																if ($addResult[0] === FALSE) {
																	...
																	if ($doDebug) {
																		echo "did unset replaceArray$advisorClass_level $replaceSequence|<br />";
																	}
...
///// all processing done. Prepare totals	
		...
		if (count($errorArray) > 0) {
 		...
 		if ($doDebug) {
 			if ($testMode) {
				$storeResult	= storeReportData_v2("TESTMODE Daily Student Cron",$content);
				$storeResult	= TRUE;
			} else {
				$storeResult	= storeReportData_v2("Daily Student Cron",$content);
			}
		...
		$storeResult	= storeReportData_v2('Daily Student Cron',$content,$testMode,$doDebug);
...
add_shortcode ('daily_student_cron_v3', 'daily_student_cron_v3_func');
