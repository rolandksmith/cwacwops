function push_advisor_class_v2_func() {
...
*/
	if ("1" == $strPass) {
		$content .= "<h3>$jobname</h3>
					...
					<tr><td>Select One</td><td><select name='request_type' class='formInputSelect' size='2' required='required'>
						<option selected='selected' value='Full'>Display & Send Full Class</option>
						<option value='sonly'>Display & Send S Students only</option>
						</select></td></tr>
					$testModeOption
					<tr><td>&nbsp;</td><td><input class='formInputButton' type='submit' value='Next' /></td></tr></table>
					</form>
					<p><u>Display & Send Full Class</u>: shows all students assigned and will send full information to the advisor.</p>
					<p><u>Display & Send S Students only:</u> shows only students who have not confirmed attendance and sends only 
					that information to the advisor.</p>";
		...
		if ($doDebug) {
			echo "<br />Got to pass 2 with request_info: $request_info and semester: $inp_semester. request_info: $request_type<br />";
		}
		if ($request_type == 'Full') {
			$theStatment	= "<p>Sending full class</p>";
		} elseif ($request_type == 'sonly') {
			$theStatment	= "<p>Sending S students only</p>";
		} else {
			$theStatment	= "<p>Nothing requested. End of process.</p>";
			$doProceed		= FALSE;
		}
		$content .= "<h3>Push Advisor Class</h3>$theStatement";
		...
		if ($doProceed) {
			...
			if ($emailsToSend > 0) {
				$content					.= "<p>Enter any additional information for the advisor(s) below:<br />
												...
												<p>Enter any additional cc or bcc to the email in the appropriate box. Format as an email address. Separate email addresses by commas. 
												for example: rolandksmith@yahoo.com,k7ojl@arrl.net<br />
												cc: <input type='text' class='formInputText' name='inp_cc' size='50' maxlength='90'><br />
												bcc: <input type='text' class='formInputText' name='inp_bcc' size='50' maxlength='90'></p>
...
// $doDebug	= TRUE;
		...
		$content .= "<h3>Advisor Class Pariticipants for $inp_semester Semester</h3>$theStatement";
				...
				if ($numARows > 0) {
					foreach ($wpw1_cwa_advisor as $advisorRow) {
						...
						if ($request_type == 'Full') {
							...
							$email_to_advisor			= $inp_msg;
						} else {
							$advisor_subject			= "CW Academy - List of Unconfirmed Students in Your Class";
							$email_to_advisor			= $inp_msg;
						}
						if ($testMode) {
						...
						if ($prepareResult[0] == FALSE) {
							...
							if ($mailResult === TRUE) {
								...
								$content .= "An email for $advisorCallSign was sent to $email_to<br /><br />";
								$myCount++;
							} else {
...
add_shortcode ('push_advisor_class_v2', 'push_advisor_class_v2_func');
