 ...
 * sends a list of students in the advisor's class(es) 
 * The advisor can choose which students to send the end-of-semester assessment email
 *
 * The email can be sent to all advisors or only those entered
 *
 * if a list of advisors is entered, put them into the advisor array
 * otherwise
 * 	Read the advisorClass pod for current semester (or previous semester, if a semester 
 * 		is not in session)
 *	check to see if the advisor is in the advisor array
 *		If not in the array, add advisor,callsign to the array
 *
 * When advisor array has been built, sort the advisor array
 * For each advisor array record
 * 		Get the advisor name and email address from the advisor pod
...
/// get the time that the process started
	...
	$studentAssessmentURL = "$siteURL/cwa-select-students-for-end-of-semester-assessment/";
	$addl_comments		= "";
...
semester and sends an email to the advisors allowing them to ask any or all of 
their students to do an end-of-semester Morse code assessment.</p>
<p>If all advisors are to receive the email, click Submit. If selected advisors are to 
receive this email, enter their call signs separated by commas and then click Submit.
...
week before the end of the semester. When you are ready to have your 
students do this assessment, please click <br />
<a href='$studentAssessmentURL?semester=$theSemester&strpass=2&inp_mode=$inp_mode&enstr=$encstr'>
<b>Select Students for Assessment</b></a>. <br />A web page will display and allow you to select 
which students to send the assessment email.<p>
<p>The Morse code assessment process has the student five Morse code segments and allow the 
student to select from a list of multiple choice answers what they heard. The student will 
be given two tries. The program will then send you an email with the score and assessment information.</p>
